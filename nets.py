...
def normal_net(tgt_image, pred_depth, is_training=True):
    ...
    with tf.variable_scope('normal_net') as sc:
        ...
        with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
                            ...
                            activation_fn=tf.nn.relu,
                            outputs_collections=end_points_collection):
            input1 = tf.concat([tgt_image, pred_depth[0]], axis = 3)
            cnv1  = slim.conv2d(input1, 32,  [7, 7], stride=2, scope='cnv1')
            cnv1b = slim.conv2d(cnv1,  32,  [7, 7], stride=1, scope='cnv1b')
            input2 = tf.concat([cnv1b, pred_depth[1]], axis = 3)
            cnv2  = slim.conv2d(input2, 64,  [5, 5], stride=2, scope='cnv2')
            cnv2b = slim.conv2d(cnv2,  64,  [5, 5], stride=1, scope='cnv2b')
            input3 = tf.concat([cnv2b, pred_depth[2]], axis = 3)
            cnv3  = slim.conv2d(input3, 128, [3, 3], stride=2, scope='cnv3')
            cnv3b = slim.conv2d(cnv3,  128, [3, 3], stride=1, scope='cnv3b')
            input4 = tf.concat([cnv3b, pred_depth[3]], axis = 3)
            cnv4  = slim.conv2d(input4, 256, [3, 3], stride=2, scope='cnv4')
            cnv4b = slim.conv2d(cnv4,  256, [3, 3], stride=1, scope='cnv4b')
            cnv5  = slim.conv2d(cnv4b, 512, [3, 3], stride=2, scope='cnv5')
            cnv5b = slim.conv2d(cnv5,  512, [3, 3], stride=1, scope='cnv5b')
            cnv6  = slim.conv2d(cnv5b, 512, [3, 3], stride=2, scope='cnv6')
            ...
            icnv1  = slim.conv2d(i1_in, 16,  [3, 3], stride=1, scope='icnv1')
            norm1, _  = pred_norm(icnv1, upsample_shape=None, scope='norm1')
