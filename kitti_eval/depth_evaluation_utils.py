...
def read_pred_file_data(files, data_root):
    pred_files = []
    num_probs = 0
    for filename in files:
        file_path = os.path.join(data_root, filename.replace(".png", ".npy"))
...
def generate_depth_map(calib_dir, velo_file_name, im_shape, cam=2, interp=False, vel_depth=False):
    ...
    dupe_inds = [item for item, count in Counter(inds).items() if count > 1] # adapt to py3
    for dd in dupe_inds:
